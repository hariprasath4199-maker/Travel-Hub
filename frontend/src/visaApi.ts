...
export const createVisaRequest = async (data: any): Promise<VisaRequest> => {
  if (!(await isBackendAvailable())) {
    // Demo mode: create request locally and persist to localStorage
...
export const sendCostCentreApproval = (id: string, data: { costCentreOwnerName: string; costCentreOwnerEmail: string; performedBy: string }): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/send-cost-centre-approval`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => json(r));

export const submitCostCentreDecision = (id: string, decision: 'approve' | 'reject', reason?: string, comments?: string): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/cost-centre-decision`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ decision, reason, comments }) }).then(r => json(r));

export const sendVendorRequest = (id: string, data: { vendorName: string; vendorEmail: string; performedBy: string }): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/vendor-request`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => json(r));

export const submitVendorDates = (id: string, dates: VendorDateSlot[], comments?: string): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/vendor-dates`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dates, comments }) }).then(r => json(r));

export const shareDatesWithApplicant = (id: string, performedBy: string): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/share-dates`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ performedBy }) }).then(r => json(r));

export const submitApplicantDates = (id: string, selectedDates: VendorDateSlot[], remarks?: string): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/applicant-dates`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ selectedDates, remarks }) }).then(r => json(r));

export const sendEvpApproval = (id: string, data: { evpName: string; evpEmail: string; performedBy: string }): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/send-evp-approval`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }).then(r => json(r));

export const submitEvpDecision = (id: string, decision: 'approve' | 'reject', reason?: string, comments?: string): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/evp-decision`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ decision, reason, comments }) }).then(r => json(r));

export const confirmBooking = (id: string, confirmedDate: VendorDateSlot, performedBy: string): Promise<VisaRequest> =>
  fetch(`${API}/visa-requests/${id}/confirm-booking`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ confirmedDate, performedBy }) }).then(r => json(r));
...
export const uploadFile = async (file: File): Promise<{ filename: string; originalName: string; size: number }> => {
  if (!(await isBackendAvailable())) {
    // Demo mode: return a mock upload response without hitting the backend
    return {
