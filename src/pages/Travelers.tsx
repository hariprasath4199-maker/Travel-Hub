...
import { MOCK_TRAVELERS } from '@/src/constants';
    ...
    <div className="p-10 space-y-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex justify-between items-end mb-10">
        <div>
          ...
          <p className="text-on-surface-variant font-medium">Directory of 124 active managed employees</p>
        </div>
        <div className="flex gap-3">
          <button className="px-4 py-2 bg-surface-container-high text-on-secondary-container rounded-lg font-semibold text-sm flex items-center gap-2 hover:bg-surface-container transition-colors">
            <Filter size={16} />
            Filters
          </button>
          <button className="px-4 py-2 bg-surface-container-high text-on-secondary-container rounded-lg font-semibold text-sm flex items-center gap-2 hover:bg-surface-container transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
        {MOCK_TRAVELERS.map((traveler) => (
          <div key={traveler.id} className="bg-surface-container-low rounded-2xl p-6 group transition-all duration-300 hover:bg-surface-container border border-transparent hover:border-outline-variant/10 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="bg-surface-container-lowest p-4 rounded-xl shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {traveler.preferences.map(pref => (
                    <span key={pref} className="bg-secondary-container text-on-secondary-container px-2 py-1 rounded text-[11px] font-semibold">
                      {pref}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between px-2">
                <div className="flex gap-3">
                  <div className="flex flex-col">
                    ...
                    <span className={`text-xs font-semibold flex items-center gap-1 ${traveler.passportStatus === 'Valid' ? 'text-tertiary' : 'text-error'}`}>
                      {traveler.passportStatus === 'Valid' ? <BadgeCheck size={14} /> : <AlertCircle size={14} />}
                      {traveler.passportStatus}
                    </span>
                  </div>
                  {traveler.visaStatus !== 'None' && (
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-semibold text-tertiary flex items-center gap-1">
                        <BadgeCheck size={14} /> {traveler.visaStatus}
                      </span>
                    </div>
                  )}
                </div>
                <div className="flex -space-x-2">
                  ...
                  {traveler.preferences.includes('Delta Diamond') && (
                    <div className="w-8 h-8 rounded-full bg-surface-container-highest border-2 border-surface-container-low flex items-center justify-center">
                      <Star size={14} />
                    </div>
                  )}
                  {traveler.preferences.includes('Car Rental Req.') && (
                    <div className="w-8 h-8 rounded-full bg-surface-container-highest border-2 border-surface-container-low flex items-center justify-center">
                      <Car size={14} />
                    </div>
                  )}
            ...
            <div className="mt-6 pt-6 border-t border-outline-variant/10 flex justify-between items-center">
              ...
              <button className="p-2 rounded-lg bg-surface-container-high text-on-surface hover:bg-primary hover:text-on-primary transition-all">
        ...
        <div className="border-2 border-dashed border-outline-variant/30 rounded-2xl p-6 flex flex-col items-center justify-center text-on-surface-variant hover:border-primary/50 transition-all group cursor-pointer shadow-sm">
          <div className="w-12 h-12 rounded-full bg-surface-container flex items-center justify-center mb-4 group-hover:bg-primary group-hover:text-on-primary transition-all">
            <UserPlus size={24} />
          </div>
          <span className="font-bold text-sm">Add New Traveler</span>
          <p className="text-[11px] text-center mt-1 px-8">Quickly import profile via HR portal or manual entry.</p>
        </div>
      </div>

      {/* Stats & Insights Footer */}
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 bg-surface-container-lowest p-8 rounded-3xl shadow-sm flex flex-col md:flex-row gap-10 items-center">
          <div className="flex-1">
            ...
            <h3 className="text-2xl font-extrabold text-on-surface mb-4">94% Directory Compliance</h3>
            <p className="text-sm text-on-surface-variant leading-relaxed max-w-md">
              The majority of your travelers have updated passports and visas. 6 profiles require immediate attention before the Q4 regional summit.
            </p>
            <div className="mt-6 flex gap-4">
              <button className="text-sm font-bold text-on-primary bg-primary px-6 py-2.5 rounded-lg shadow-lg shadow-primary/20 hover:bg-primary-dim transition-all active:scale-95">Send reminders</button>
              <button className="text-sm font-bold text-primary border border-primary/20 px-6 py-2.5 rounded-lg hover:bg-primary/5 transition-all">View detailed report</button>
            </div>
          </div>
            ...
            <div className="relative flex flex-col items-center">
              <div className="relative w-32 h-32 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle className="text-primary" cx="64" cy="64" fill="transparent" r="58" stroke="currentColor" strokeDasharray="364.4" strokeDashoffset="21.8" strokeWidth="12"></circle>
                </svg>
                <span className="absolute text-2xl font-black text-on-surface">94%</span>
              </div>
              <span className="mt-3 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Global Status</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-primary to-primary-dim p-8 rounded-3xl text-on-primary flex flex-col justify-between shadow-lg shadow-primary/20">
          <div>
            <PlaneTakeoff size={40} className="opacity-50 mb-4" />
            <h4 className="text-4xl font-black mb-1">12</h4>
            <p className="text-xs font-medium opacity-80 leading-tight">Travelers currently in transit across 4 continents.</p>
          </div>
          <button className="w-full py-2 bg-on-primary/10 hover:bg-on-primary/20 rounded-lg text-xs font-bold uppercase tracking-widest transition-all">Live Map</button>
        </div>
      </div>
    </div>
  );
